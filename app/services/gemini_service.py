...
class GeminiService:
    ...
    def summarize_turn(self, user_text: str, answer_text: str) -> str:
        ...
        prompt = (
            ...
            'Giữ các dữ kiện quan trọng, task hiện tại và các ảnh được nhắc tới.\n\n'
            f'User: {user_text}\nAssistant: {answer_text}'
        )
        result = self._generate_json(prompt, schema)
        return result.get('summary', '')

    def update_working_memory(self, previous_memory: dict[str, Any], user_text: str, image_summaries: list[dict[str, Any]]) -> dict[str, Any]:
        schema = {
            'type': 'object',
            'required': [
        ...
        prompt = (
            'Cập nhật working memory cho hệ thống hội thoại đa phương thức. '
            'Giữ trạng thái hiện tại của task, current focus, active images, unresolved questions.\n\n'
